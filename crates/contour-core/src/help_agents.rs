...
pub struct SkillInstallOptions<'a> {
    ...
    /// Replace a legacy (unmarkered) contour section instead of skipping it.
    /// Marker-wrapped sections always refresh; this only affects blocks from
    /// installs that predate the markers, whose end boundary is heuristic
    /// (section start to end of file).
    pub force: bool,
}
...
/// block is contour-owned: refresh replaces it in place, content outside
/// survives.
const SKILL_BEGIN: &str = "<!-- contour:skill:begin -->";
const SKILL_END: &str = "<!-- contour:skill:end -->";

/// What [`merge_skill_into`] did with the existing agent-file body.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum MergeOutcome {
    /// No contour block found — skill appended, wrapped in markers.
    Appended,
    /// An existing block (markered, or legacy under `force`) was replaced.
    Replaced,
    /// A legacy (unmarkered) block exists and `force` was not given —
    /// file untouched.
    SkippedLegacy,
}

/// Merge the rendered skill into an existing agent-file body.
///
/// - Markered block (`<!-- contour:skill:begin/end -->`): replaced in place,
///   regardless of `force` — the markers delimit contour-owned content.
/// - Legacy block (recognizable contour content, no markers): replaced from
///   its start **to EOF** only under `force` (legacy installs always appended
///   the block last, but the boundary is a heuristic — hence the opt-in).
///   Without `force`, returns [`MergeOutcome::SkippedLegacy`] and no body.
/// - Neither: skill appended, wrapped in markers so future refreshes replace.
fn merge_skill_into(existing: &str, skill: &str, force: bool) -> (Option<String>, MergeOutcome) {
    let markered_block = format!("{SKILL_BEGIN}\n{skill}\n{SKILL_END}");

    if let Some(begin) = existing.find(SKILL_BEGIN) {
        if let Some(end_at) = existing[begin..].find(SKILL_END) {
            let after = begin + end_at + SKILL_END.len();
            let body = format!(
                "{}{markered_block}{}",
                &existing[..begin],
                &existing[after..]
            );
            return (Some(body), MergeOutcome::Replaced);
        }
        // Begin without end: treat as legacy (boundary unknown).
    }

    // Legacy signatures: skill frontmatter, the generation stamp, or the
    // pre-frontmatter template's title line.
    let legacy_start = ["name: contour", "<!-- Generated by contour"]
        .iter()
        .filter_map(|sig| existing.find(sig))
        .min()
        .or_else(|| existing.find("contour — macOS MDM Configuration Toolkit"));

    let Some(sig_at) = legacy_start else {
        // No contour content — append with markers.
        ...
        body.push_str(&markered_block);
        body.push('\n');
        return (Some(body), MergeOutcome::Appended);
    };

    if !force {
        return (None, MergeOutcome::SkippedLegacy);
    }

    // Block start: the `---` frontmatter opener directly above the signature
    // when present, else the signature's own line.
    let line_start = existing[..sig_at].rfind('\n').map_or(0, |i| i + 1);
    let block_start = {
        let before = existing[..line_start].trim_end_matches('\n');
        if before.lines().next_back() == Some("---") {
            let opener_at = before.len() - 3;
            existing[..opener_at].rfind('\n').map_or(0, |i| i + 1)
        } else {
            line_start
        }
    };

    let mut body = existing[..block_start].trim_end_matches('\n').to_string();
    if !body.is_empty() {
        body.push_str("\n\n");
    }
    body.push_str(&markered_block);
    body.push('\n');
    (Some(body), MergeOutcome::Replaced)
}
...
pub fn install_skill_with(version: &str, opts: &SkillInstallOptions<'_>) -> Result<()> {
    ...
    for agent_file in &agent_files {
        ...
        if path.exists() {
            ...
            match merge_skill_into(&existing, &skill_content, opts.force) {
                (Some(body), MergeOutcome::Appended) => {
                    fs::write(path, body)?;
                    eprintln!("\u{2713} Added contour instructions to {agent_file}");
                }
                (Some(body), MergeOutcome::Replaced) => {
                    fs::write(path, body)?;
                    eprintln!("\u{2713} Refreshed contour instructions in {agent_file}");
                }
                (_, MergeOutcome::SkippedLegacy) => {
                    eprintln!(
                        "  {agent_file} has a contour section from an older install — \
                         re-run with --force to replace it (replaces from the section \
                         start to end of file)"
                    );
                }
                (None, _) => unreachable!("merge returns a body unless skipping"),
            }
        } else {
            // Create with the markered block so future refreshes replace in place.
            fs::write(
                path,
                format!("{SKILL_BEGIN}\n{skill_content}\n{SKILL_END}\n"),
            )?;
...
mod skill_tests {
    ...
    #[test]
    fn merge_into_empty_appends_with_markers() {
        let (body, outcome) = merge_skill_into("# My prefs\n", "SKILL BODY", false);
        let body = body.expect("append should produce a body");
        assert_eq!(outcome, MergeOutcome::Appended);
        assert!(body.starts_with("# My prefs\n"));
        assert!(body.contains("<!-- contour:skill:begin -->"));
        assert!(body.contains("SKILL BODY"));
        assert!(body.trim_end().ends_with("<!-- contour:skill:end -->"));
    }

    /// A markered block is contour-owned — refresh replaces it without
    /// --force, and content after the end marker survives.
    #[test]
    fn markered_block_refreshes_without_force() {
        let existing = "# My prefs\n\n<!-- contour:skill:begin -->\nOLD SKILL\n<!-- contour:skill:end -->\n\n# My notes after\n";
        let (body, outcome) = merge_skill_into(existing, "NEW SKILL", false);
        let body = body.expect("markered block should refresh");
        assert_eq!(outcome, MergeOutcome::Replaced);
        assert!(body.starts_with("# My prefs\n"));
        assert!(body.contains("NEW SKILL"));
        assert!(!body.contains("OLD SKILL"));
        assert!(body.contains("# My notes after"));
        assert_eq!(body.matches("<!-- contour:skill:begin -->").count(), 1);
    }

    /// Legacy (unmarkered) blocks have no reliable end boundary — without
    /// --force they are left alone, never duplicated.
    #[test]
    fn legacy_block_skipped_without_force() {
        let existing =
            "# My prefs\n\n---\nname: contour\ndescription: old\n---\n\n# contour\nOLD CONTENT\n";
        let (body, outcome) = merge_skill_into(existing, "NEW SKILL", false);
        assert_eq!(outcome, MergeOutcome::SkippedLegacy);
        assert!(body.is_none(), "skip must not rewrite the file");
    }

    /// With --force, the legacy block (frontmatter opener through EOF —
    /// legacy installs always appended last) is replaced by a markered
    /// fresh block; content BEFORE the block survives.
    #[test]
    fn legacy_block_replaced_with_force() {
        let existing = "# My prefs\nkeep me\n\n---\nname: contour\ndescription: old\n---\n\n# contour\nOLD CONTENT\n";
        let (body, outcome) = merge_skill_into(existing, "NEW SKILL", true);
        let body = body.expect("force should replace the legacy block");
        assert_eq!(outcome, MergeOutcome::Replaced);
        assert!(body.starts_with("# My prefs\nkeep me\n"));
        assert!(!body.contains("OLD CONTENT"));
        assert!(!body.contains("description: old"));
        assert!(body.contains("NEW SKILL"));
        assert!(
            body.contains("<!-- contour:skill:begin -->"),
            "forced replacement must leave markers so the next refresh is automatic"
        );
    }
