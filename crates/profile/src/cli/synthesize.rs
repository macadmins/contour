...
pub fn handle_synthesize(
    ...
    // Phase 6: optional Markdown key reference
    if let Some(md_path) = keys_md {
        let apple_sha = mdm_schema::schema_versions().apple_device_management_commit;
        let md = render_keys_markdown(&selected, &registry, &apple_sha);
        if dry_run {
            if mode == OutputMode::Human {
                println!(
                    "  {} Would write key reference: {}",
                    "~".cyan(),
                    md_path.display()
                );
            }
            ...
            if mode == OutputMode::Human {
                println!("  {} Key reference -> {}", "->".green(), md_path.display());
            }
        }
    }
...
// ── Markdown key reference ───────────────────────────────────────────

/// Build a Markdown reference for the synthesized profiles: one section per
/// payload domain, a table of every key with its type/required/description,
/// and a back-link to Apple's source for `com.apple.*` types.
fn render_keys_markdown(
    plists: &[&DiscoveredPlist],
    registry: &SchemaRegistry,
    apple_sha: &str,
) -> String {
    let mut out = String::new();
    out.push_str("# Synthesized configuration keys\n\n");
    out.push_str(&format!(
        "{} payload(s). Generated by `contour profile synthesize`.\n\n",
        plists.len()
    ));

    for entry in plists {
        out.push_str(&format!("## {}\n\n", entry.domain));
        if !entry.title.is_empty() && entry.title != entry.domain {
            out.push_str(&format!("_{}_\n\n", md_inline(&entry.title)));
        }
        match apple_source_link(&entry.domain, apple_sha) {
            Some(link) => out.push_str(&format!("Source: {link}\n\n")),
            None => out.push_str("Source: vendor-defined (no Apple schema reference)\n\n"),
        }
        if !entry.schema_matched {
            out.push_str("> No Apple schema match — key descriptions unavailable.\n\n");
        }

        out.push_str("| Key | Type | Required | Description |\n");
        out.push_str("|-----|------|----------|-------------|\n");

        let manifest = registry.get(&entry.domain);
        for key in entry.dict.keys() {
            let field = manifest.and_then(|m| m.fields.get(key));
            let (ty, required, desc) = match field {
                Some(f) => (
                    f.field_type.as_str().to_string(),
                    if f.flags.required { "yes" } else { "" },
                    md_inline(&f.description),
                ),
                None => (
                    entry
                        .dict
                        .get(key)
                        .map_or("?", plist_value_type_name)
                        .to_string(),
                    "",
                    String::new(),
                ),
            };
            out.push_str(&format!("| `{key}` | {ty} | {required} | {desc} |\n"));
        }
        out.push('\n');
    }

    if !apple_sha.is_empty() {
        let short = &apple_sha[..7.min(apple_sha.len())];
        out.push_str(&format!(
            "---\n_Apple schema: [apple/device-management @ {short}]\
             (https://github.com/apple/device-management/tree/{apple_sha}) · \
             plus profilecreator manifests_\n",
        ));
    }
...
/// Apple-source back-link for a payload domain, gated on the `com.apple.`
/// prefix (only those types live in Apple's `device-management` repo). Uses a
/// code-search URL rather than a direct blob path so it always resolves even
/// when Apple's file layout differs from the type name. `None` for vendor or
/// community domains.
fn apple_source_link(domain: &str, apple_sha: &str) -> Option<String> {
    if !domain.starts_with("com.apple.") {
        return None;
    }
    let url = format!(
        "https://github.com/search?q=repo%3Aapple%2Fdevice-management+%22{domain}%22&type=code"
    );
    if apple_sha.is_empty() {
        Some(format!("[apple]({url})"))
    } else {
        let short = &apple_sha[..7.min(apple_sha.len())];
        Some(format!("[apple @ {short}]({url})"))
    }
}

/// Collapse a schema description into a single Markdown table cell — flattens
/// whitespace/newlines and escapes pipes so the table stays well-formed.
...
mod tests {
    ...
    #[test]
    fn apple_source_link_includes_short_sha_and_repo_search() {
        let link = apple_source_link("com.apple.wifi.managed", "a1b2c3d4e5").unwrap();
        assert!(link.contains("apple @ a1b2c3d"), "short sha: {link}");
        assert!(link.contains("repo%3Aapple%2Fdevice-management"));
        assert!(link.contains("com.apple.wifi.managed"));
    }

    #[test]
    fn apple_source_link_none_for_vendor_domain() {
        assert!(apple_source_link("com.foocorp.app", "a1b2c3d4").is_none());
    }

    #[test]
    fn apple_source_link_omits_at_when_sha_empty() {
        let link = apple_source_link("com.apple.dock", "").unwrap();
        assert!(link.starts_with("[apple]("), "{link}");
        assert!(!link.contains('@'));
    }

    #[test]
    fn md_inline_escapes_pipes_and_collapses_whitespace() {
        assert_eq!(md_inline("a | b\n  c"), "a \\| b c");
    }

    #[test]
    fn render_keys_markdown_builds_table_and_provenance() {
        let registry = SchemaRegistry::embedded().expect("schemas");
        let mut dict = Dictionary::new();
        dict.insert("allowCamera".to_string(), Value::Boolean(false));
        let entry = DiscoveredPlist {
            domain: "com.apple.applicationaccess".to_string(),
            dict,
            schema_matched: true,
            title: "Restrictions".to_string(),
        };
        let md = render_keys_markdown(&[&entry], &registry, "deadbeef");
        assert!(md.contains("## com.apple.applicationaccess"));
        assert!(md.contains("| `allowCamera` |"));
        assert!(md.contains("apple @ deadbee"));
        assert!(md.contains("device-management/tree/deadbeef"));
    }

    #[test]
    fn render_keys_markdown_marks_unmatched_vendor_payload() {
        let registry = SchemaRegistry::embedded().expect("schemas");
        let mut dict = Dictionary::new();
        dict.insert("CustomKey".to_string(), Value::String("x".to_string()));
        let entry = DiscoveredPlist {
            domain: "com.foocorp.app".to_string(),
            dict,
            schema_matched: false,
            title: "com.foocorp.app".to_string(),
        };
        let md = render_keys_markdown(&[&entry], &registry, "deadbeef");
        assert!(md.contains("vendor-defined"));
        assert!(md.contains("No Apple schema match"));
        assert!(md.contains("| `CustomKey` |"));
    }
}
