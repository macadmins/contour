...
fn main() {
    ...
    println!("cargo:rerun-if-changed=data/app_policies.parquet");
    ...
    if data_dir.join("app_policies.parquet").exists() {
        return;
    }

    let url = std::env::var("CONTOUR_APP_POLICY_SCHEMA_URL").unwrap_or_else(|_| {
        panic!(
            "CONTOUR_APP_POLICY_SCHEMA_URL is not set and data/app_policies.parquet is missing.\n\
             Set CONTOUR_APP_POLICY_SCHEMA_URL to the URL of app-policy-schema.zip,\n\
             or copy parquet files into crates/app-policy-schema/data/ manually\n\
             (the posture pipeline's publish-datasets.sh does this)."
        )
    });

    download_and_extract(&url, data_dir, "app-policy-schema/data");
}

/// Download a zip archive from `url`, extract it, and move files from the
/// nested `inner_prefix` directory into `data_dir`.
///
/// The upstream zips contain the full crate layout (e.g. `app-policy-schema/data/*.parquet`),
