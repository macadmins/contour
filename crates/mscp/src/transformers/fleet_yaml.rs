// Fleet YAML transformers - public API
#![allow(dead_code, reason = "module under development")]

use crate::models::{
    BaselineReference, Controls, CustomSetting, FleetConfig, MscpBaseline, Platform,
    PlatformSettings, Script,
};
use anyhow::Result;
use std::path::{Path, PathBuf};

/// Generator for `FleetDM` fleet YAML files and baseline components
#[derive(Debug)]
pub struct FleetYamlGenerator {
    output_base: PathBuf,
}

impl FleetYamlGenerator {
    ...
    pub fn write_baseline_component(
        ...
        // Convert FleetConfig to BaselineReference (TOML format)
        let baseline_ref =
    ...
    /// `fleets/` directory.
    pub fn generate_fleet_yaml(&self, baseline_name: &str) -> Result<String> {
        ...
        let content = format!(
            r#"# Fleet GitOps - Fleet Configuration: {fleet_name} (Fleet v4.83+)
#
...
"#,
    ...
    pub fn write_fleet_yml(&self, baseline_name: &str) -> Result<PathBuf> {
        ...
        let content = self.generate_fleet_yaml(baseline_name)?;
    ...
    /// Write fleet YAML with actual profiles and scripts populated
    pub fn write_fleet_yml_with_content(
        &self,
        baseline_name: &str,
        profile_paths: &[PathBuf],
        ...
        generator.generate_fleet_yml(baseline_name, profile_paths, script_paths)
    }

    /// Deprecated: Keep for backward compatibility
    #[deprecated(since = "0.2.0", note = "Use write_fleet_yml instead")]
    pub fn write_example_fleet(&self, baseline_name: &str) -> Result<PathBuf> {
        self.write_fleet_yml(baseline_name)
    }
