...
fn main() {
    ...
    println!("cargo:rerun-if-changed=data/rules_versioned.parquet");
    ...
    if data_dir.join("rules_versioned.parquet").exists() {
        return;
    }

    let url = std::env::var("CONTOUR_MSCP_SCHEMA_URL").unwrap_or_else(|_| {
        panic!(
            "CONTOUR_MSCP_SCHEMA_URL is not set and data/rules_versioned.parquet is missing.\n\
             Set CONTOUR_MSCP_SCHEMA_URL to the URL of mscp-schema.zip,\n\
             or copy parquet files into crates/mscp-schema/data/ manually."
        )
    });

    download_and_extract(&url, data_dir, "mscp-schema/data");
}

/// Download a zip archive from `url`, extract it, and move files from the
/// nested `inner_prefix` directory into `data_dir`.
///
/// The upstream zips contain the full crate layout (e.g. `mscp-schema/data/*.parquet`),
