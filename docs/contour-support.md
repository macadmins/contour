...
`init` scans a parent directory of brand subfolders (e.g. `acme/`, `globex/`,
`initech/`) — each holding that brand's logo, dark-mode logo, and menu-bar
...
```bash
contour support generate support.toml -o ./profiles
contour support generate support.toml --brand globex --dry-run
```
